...
def run_core(networks):
    ...
    df=pd.DataFrame(rows); df.to_csv(RESULTS/"core_scenarios.csv",index=False); return df
...
def main():
    ...
    if a.suite=="full":
        ...
        rs=pd.DataFrame(summaries); rs.to_csv(RESULTS/"N5_robustness_summary.csv",index=False)
        print("[robustness]"); print(rs.to_string(index=False))

        prop=run_property_tests(a.trials,a.seed); prop.to_csv(RESULTS/"property_tests.csv",index=False)
        ps=pd.DataFrame([{"trials":len(prop),"bounded_pass":int(prop.bounded.sum()),"monotone_B_pass":int(prop.monotone_B.sum()),
                          "B1_implies_R1_pass":int(prop.B1_implies_R1.sum()),"S0_gates_edge_pass":int(prop.S0_gates_edge.sum())}])
        ps.to_csv(RESULTS/"property_tests_summary.csv",index=False)
        print("[property tests]"); print(ps.to_string(index=False))

        sc=run_scalability(); sc.to_csv(RESULTS/"scalability.csv",index=False)
        print("[scalability]"); print(sc.to_string(index=False))
